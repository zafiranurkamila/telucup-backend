        ...
        <div class="mt-6">
            ...
            >
                    ...
                    <template x-if="!loading && error">
                        <div class="flex flex-col items-center justify-center h-64 text-center gap-4">
                            <div class="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center">
                                <svg class="w-7 h-7 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                            </div>
                            <div>
                                ...
                                >
                                    ...
                                    <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/65 to-transparent px-3 py-2.5 flex items-end justify-between gap-1.5">
                                        <span
                                            ...
                                            :class="similarityClass(photo.similarity_score)"
                                            x-text="photo.similarity_score ? Math.round(photo.similarity_score * 100) + '%' : '—'"
                                        ></span>
                                        <span
                                            class="text-[10px] font-bold px-2 py-0.5 rounded-full border"
                ...
                <template x-if="selectedPhoto">
                    ...
                    >
                        <div class="bg-white rounded-2xl overflow-hidden w-full max-w-2xl shadow-2xl flex flex-col max-h-[92vh]"
                             x-transition:enter="transition ease-out duration-200"
                            ...
                            <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-100 shrink-0">
                                <h3 class="text-sm font-bold text-gray-800 flex items-center gap-2">
                                    <svg class="w-4 h-4 text-[#B41F2A]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/></svg>
                                    Detail Foto
                                </h3>
                            ...
                            <div class="relative bg-black overflow-hidden shrink-0">
                                <img
                                    ...
                                    @load="computeBbox()"
                                >
                                <template x-if="bboxStyle">
                                    <div class="absolute border-[3px] border-yellow-400 pointer-events-none" :style="bboxStyle">
                                        <span class="absolute -top-6 left-0 bg-yellow-400 text-black text-[10px] font-bold px-1.5 py-0.5 rounded leading-none whitespace-nowrap">Wajah Terdeteksi</span>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="p-5 space-y-4 overflow-y-auto">
                                ...
                                <template x-if="selectedPhoto.validation_status === 'rejected'">
                                    <div class="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                                        <svg class="w-5 h-5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('fotoSayaGallery', () => ({
        ...
        get filteredPhotos() {
            ...
            return this.photos.filter(p => p.validation_status === this.statusFilter);
        },
        ...
        async validate(photoFaceId, status) {
            ...
            try {
                ...
                if (res.ok) {
                    const idx = this.photos.findIndex(p => p.id === photoFaceId);
                    if (idx !== -1) this.photos[idx].validation_status = status;
        ...
        computeBbox() {
            ...
            const bb  = this.selectedPhoto?.bounding_box;
            if (!bb || !img?.naturalWidth) return;
            ...
            const offX = (img.clientWidth  - renderedW) / 2;
            const offY = (img.clientHeight - renderedH) / 2;

            const x = bb.x              * scale + offX;
            const y = bb.y              * scale + offY;
            const w = (bb.width  ?? bb.w) * scale;
            const h = (bb.height ?? bb.h) * scale;

            this.bboxStyle = `left:${x}px;top:${y}px;width:${w}px;height:${h}px;`;
        },
