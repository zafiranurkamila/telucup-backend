...
<div class="space-y-2.5">
    ...
    <label class="flex min-h-12 cursor-pointer items-center gap-3 rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm font-semibold text-gray-700 shadow-sm transition-all hover:border-[#B41F2A]/40 hover:bg-red-50/40"
           :class="{ 'border-[#B41F2A] bg-red-50 text-[#B41F2A] ring-1 ring-[#B41F2A]/20': answers['{{ $question['code'] }}'] && answers['{{ $question['code'] }}'].includes('{{ $val }}') }">
        <input type="checkbox"
               name="{{ $question['code'] }}[]"
               ...
               class="h-4 w-4 rounded border-gray-300 accent-[#B41F2A] focus:ring-[#B41F2A]" />
