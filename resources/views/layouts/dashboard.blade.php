    ...
    <div x-data="{ sidebarOpen: false }" class="flex flex-col h-screen overflow-hidden">
        ...
        <header class="h-[60px] bg-[#a81d22] text-white flex items-center justify-between px-4 lg:px-6 shadow-md shrink-0 z-50 relative">
            ...
            <div class="flex items-center gap-3">
                ...
                @php
                    $userPhoto = Auth::user()->player?->photo_path;
                @endphp
                <div class="w-8 h-8 rounded bg-[#89a2cc] flex items-center justify-center border border-white/20 overflow-hidden shrink-0">
                    @if($userPhoto)
                        <img src="{{ str_starts_with($userPhoto, 'http') ? $userPhoto : asset('storage/' . $userPhoto) }}" alt="Profile" class="w-full h-full object-cover">
                    @else
